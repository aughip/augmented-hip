...
public:
	...
	inline Quat() : x(0.0), y(0.0), z(0.0), w(0.0) {}
	...
	static Quat lookAt(Vector3& v0, Vector3& v1) {

		Vector3 forward = v0;
		v0.normalize();
		Vector3 up = v1.orthogonal(forward); // Keeps up the same, make forward orthogonal to up
	...
	static Quat lookAt(const Vector3& v0, const Vector3& v1) {
		double lengthsq = v0.length_squared();
		Vector3 forward;
		if (lengthsq == 0) {
			...
			forward = Vector3(v0.x / length, v0.y / length, v0.z / length);
		}
		Vector3 up = v1.orthogonal(forward); // Keeps up the same, make forward orthogonal to up
		Vector3 right = up.cross(forward);
	...
	static Quat fromToRotation(const Vector3& from, const Vector3& to) {
		...
		double angle = from.angle_to(to);
		Quat quat = Quat(axis, angle);
		return quat;
	}
};
